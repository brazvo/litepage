...
class AdmProductsModel extends BaseModel
{
  // Properties
    /**
	 * @var string
  ...
  {
	// GET CATEGORY ID
	$record = db::fetch("SELECT * FROM menu_items WHERE path='$path'");
	...
	$result['products'][$cat_id] = db::fetchAll("SELECT * FROM products WHERE category_id='$cat_id' ORDER BY name");
  ...
  public function saveNew($post, $files)
  {
	...
	db::exec("INSERT INTO products (id, image) VALUES ($newid, '$image')");
  ...
  public function save($post, $files, $id)
  {
	...
	foreach($post as $key => $value){
	  if($key == 'save' or $key == 'validate' or $key == 'deleteimg'){
	    // DO NOTHING
	  }
	  else{
