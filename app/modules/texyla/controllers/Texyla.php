<?php
class Texyla extends Controller
{
  ...
  function actionSave()
  {
    if(Form::$isvalid){
		...
		if($result) redirect('texyla/admin', SAVE_OK);
		if(!$result) redirect('texyla/admin', SAVE_FAILED);
	}
	else{
  ...
  function actionDelete($id)
  {
	...
	if($result) redirect('texyla/admin', DELETED);
	if(!$result) redirect('texyla/admin', DELETE_FAILED);
  }
  ...
  function renderAdd()
  {
	...
	$this->template['link'] = Html::elem('a', array('href'=>BASEPATH.'/texyla/admin'), '[ '.TEXYLA_ADMIN_TITLE.' ]');
  ...
  {
    $form = new Form('texyla', 'texyla', BASEPATH.'/texyla/saveNew');
  ...
  {
    $form = new Form('texyla', 'texyla', BASEPATH.'/texyla/save');
