...
class AdmKontaktModel
{
  ...
  public function send($post)
  {
	...
	$to = db::fetchSingle("SELECT email FROM contact_form_settings WHERE contact_frm_id=%i", $contact_frm_id);
	$form = $row['title'];
	
	if(NOTIFICATE && $to) {
		$toEmail = trim(NOTIFICATE) .", ".trim($to);
	}
	else if (!NOTIFICATE && $to) {
		$toEmail = trim($to);
	}
	else if (NOTIFICATE && !$to) {
		$toEmail = trim(NOTIFICATE);
	}
	...
	if( isset($toEmail) ) {
		...
		$continue = Application::mailer($from, $toEmail, emailStr($subject), $mail);
		//$continue = true;
	}
	else{
		$continue = true;
  ...
  public function save($post)
  {
	...
	$result = db::exec("UPDATE contact_form_settings SET
						...
						form_after_text=$form_after_text, hide_on_load=$hide_on_load, email='$email' WHERE contact_frm_id=$id");
	if(trim($path_alias) != ''){
	  $row = db::fetch("SELECT * FROM path_aliases WHERE module='contactform' AND mod_id=$id");
	  if($row){
	    ...
	    db::exec("UPDATE path_aliases SET path_alias='$path_alias' WHERE module='contactform' AND mod_id=$id");
	  }
	  else{
	    db::exec("INSERT INTO path_aliases (path_alias, url, module, mod_id) VALUES ('$path_alias', 'contactform/frontend/show/$id', 'contactform', $id)");
	  }
	}
	else{
	  db::exec("DELETE FROM path_aliases WHERE module='contactform' AND mod_id=$id");
	}
	...
	$menuFetch = db::fetch("SELECT * FROM menu_items WHERE module='contactform' AND content_id='$id' AND menu_id='$menu_id' AND id='$child_of'");
	...
	if(trim($post['menu_title']) != ''){
		...
		trim($post['path_alias']) !='' ? $path = $post['path_alias'] : $path = "contactform/frontend/show/$id";
		$row = db::fetch("SELECT * FROM menu_items WHERE module='contactform' AND content_id='$id'");
		if($row){
			db::exec("UPDATE menu_items SET title='".$post['menu_title']."', path='$path', menu_id='$menu_id', child_of='$child_of' WHERE module='contactform' AND content_id='$id'");
		}
		else{
			db::exec("INSERT INTO menu_items VALUES (null, '".$post['menu_title']."', '$path', 1, 0, 0, '$child_of', '$menu_id', '".$post['menu_title']."', '', '$id', 'contactform')");
		}
	}
	else{
		db::exec("DELETE FROM menu_items WHERE module='contactform' AND content_id='$id'");
	}
        ...
        $sql = "UPDATE content SET `content_title`='{$title}',
                                   ...
                                   `edit_uid`={$uid},
                                   `lang`='{$lang}'
                               WHERE `content_type_machine_name`='contactform'
                                   AND `content_id`={$id}";
        db::exec($sql);
  ...
  public function saveNew($post)
  {
	...
	$result = db::exec("INSERT INTO contact_form_settings 
	                   (contact_frm_id, company_fld, company_fld_req, address_fld, address_fld_req, phone_fld, phone_fld_req,
					    email_fld, email_fld_req, subject_fld, subject_fld_req, form_after_text, hide_on_load, email) 
						VALUES ($newid, $company_fld, $company_fld_req, $address_fld, $address_fld_req, $phone_fld, $phone_fld_req,
					    $email_fld, $email_fld_req, $subject_fld, $subject_fld_req, $form_after_text, $hide_on_load, '$email')");
	if( !empty( $path_alias ) ){
	  db::exec("INSERT INTO path_aliases (path_alias, url, module, mod_id) VALUES ('$path_alias', 'contactform/frontend/show/$newid', 'contactform', $newid)");
	}
	
	//CREATE MENU ITEM
	if(trim($post['menu_title']) != ''){
		list($menu_id, $child_of) = explode(':', $post['menu_items']);
		trim($post['path_alias']) !='' ? $path = $post['path_alias'] : $path = "contactform/frontend/show/$newid";
		db::exec("INSERT INTO menu_items VALUES (null, '".$post['menu_title']."', '$path', 1, 0, 0, '$child_of', '$menu_id', '".$post['menu_title']."', '', '$newid', 'contactform')");
	}
        ...
        $sql = "INSERT INTO content (`id`, `content_type_id`, `content_type_name`, `content_title`,
				     ...
				     VALUES ('$new_cont_id', 0, 'Contact Form', '{$title}',
				     '{$path_alias}', '{$timestamp}', 'contactform',
  ...
  public function delete($id)
  {
	...
	$result = db::exec("DELETE FROM path_aliases WHERE module='contactform' AND mod_id=$id");
	return $result;
  }
