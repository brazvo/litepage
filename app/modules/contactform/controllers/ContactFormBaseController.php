...
class ContactFormBaseController extends Controller {
    ...
    {
        Application::setPermissions(Application::$logged['role'], 'contactform');
        
        $this->template->bodyclass = 'module_contactform';
        
        $contMod = new ContentModel;

        if( $this->id ) {
            $this->cid = $contMod->getContentId('contactform', $this->id);
        }
        else {
            $this->cid = -1;
        }
