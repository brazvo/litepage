...
class AdmKontaktModel
{
  ...
  {
    $row = db::fetch("SELECT * FROM contact_forms WHERE id=$id");
	return $row;
  }
  ...
  {
    $rows = db::fetchAll("SELECT * FROM contact_forms ORDER BY title");
	return $rows;
  }
  ...
  public function send($post)
  {
	...
	$row = db::fetch("SELECT title FROM contact_forms WHERE id=$contact_frm_id");
	$form = $row['title'];
  ...
  public function save($post)
  {
	...
	$result = db::exec("UPDATE contact_forms SET title='$title', form_title='$form_title', body='$body', path_alias='$path_alias', lang='$lang', updated='$timestamp' WHERE id=$id");
  ...
  public function saveNew($post)
  {
	...
	$row = db::fetch("SELECT MAX(id) AS id FROM contact_forms");
	$newid = $row['id']+1;
	$body = stripslashes($body);
	
	$timestamp = DATE("Y-m-d H:i:s");
	$result = db::exec("INSERT INTO contact_forms (id, title, form_title, body, path_alias, lang, updated) VALUES ($newid, '$title', '$form_title', '$body', '$path_alias', '$lang', '$timestamp')");
  ...
  {
    $result = db::exec("DELETE FROM contact_forms WHERE id=$id");
  ...
  {
    $rows = db::fetchAll("SELECT id, title FROM contact_forms");
